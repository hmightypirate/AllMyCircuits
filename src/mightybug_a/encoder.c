...
void update_velocities_encoders(void)
{
  if (measures_done > 0)
  {
    ...
    right_velocity = (right_encoder * WHEEL_PERIMETER) / WHEEL_RATIO;

    /* obtain acceleration info */
    update_acc_encoders((int32_t)left_encoder - last_left_encoder,
                        (int32_t)right_encoder - last_right_encoder);
...
void update_encoder_ticks()
{
  ...
  left_encoder_array[encoder_array_index] = encoder_measurement(timer_tmp);

  // Get right encoder
  timer_tmp = (uint32_t)timer_get_counter(RIGHT_ENCODER_TIMER);
  right_encoder_array[encoder_array_index] = encoder_measurement(timer_tmp);
