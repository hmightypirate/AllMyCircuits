...
sed -i 's/^#define SYSTICK_MEAS.*$/#define SYSTICK_MEAS 10/' ../src/mightybug_a/encoder.h

# turbo state
sed -i 's/#define OUT_TURBO_HYST.*/#define OUT_TURBO_HYST USE_ENCODERS_FOR_STATE? 4:20/' ../src/mightybug_a/fsm.h

# mappings
sed -i 's/^.*mapping vel normal/700, 750, 780 \/\/ mapping vel normal/' ../src/mightybug_a/fsm.c 

sed -i 's/^.*mapping vel turbo/625, 650, 675 \/\/ mapping vel turbo/' ../src/mightybug_a/fsm.c 
